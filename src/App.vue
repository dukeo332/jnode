...
// Define a set of characters that tend to cause issues in CSVs or shell scripts
// Expanded to include single/double quotes and other shell/CSV-problematic symbols
const bannedSymbols = new Set([
  "'", '"', ',', '\\', '|', '&', '$', '%', ';', '<', '>',
  '#', '*', '?', '{', '}', '~', '`', ':', '(', ')', '^', '=', '/'
])

// Original symbol pools filtered to remove banned/problematic characters
let symbolsNonShift = ['`', '-', '=', '[', ']', '\\', ';', "'", ',', '.', '/'].filter(s => !bannedSymbols.has(s))
let symbolsShift = ['~','!','@','#','$','%','^','&','*','(',')','_','+','{','}','|',':','"','<','>','?'].filter(s => !bannedSymbols.has(s))

// Provide safe fallbacks if filtering removed everything
if (symbolsNonShift.length === 0) symbolsNonShift = ['-', '.', '_']
if (symbolsShift.length === 0) symbolsShift = ['!', '@', '_', '+']
...
// Sanitize final password by stripping any banned/problematic characters (extra safety)
const sanitizePassword = (s) => {
  // remove any occurrence of banned characters (keeps the list in sync with bannedSymbols)
  return s.replace(/['",\\|&$%;<>#*\?{}\~`\:\(\)\^=\/]/g, '')
}
...
const generateSinglePassword = () => {
  ...
  while (true) {
    ...
    // Sanitize the assembled password (removes problematic characters if present)
    const cleaned = sanitizePassword(assembledPassword)
